...
impl MarketBus {
    pub fn new() -> Self {
        ...
        Self { candles_1h: c1h, candles_4h: c4h, order_book: ob }
    }
}
...
enum FeedSecurity {
    ...
    Hmac { header: &'static str, secret_env: &'static str },
}
...
async fn binance_feed(bus: Arc<MarketBus>, sec: FeedSecurity) {
    ...
    let (mut ws, _) = match connect_async(url).await {
        Ok(t) => t,
        Err(e) => { log::error!("binance ws connect: {e}"); return; },
    };

    while let Some(Ok(msg)) = ws.next().await {
        if let Message::Text(txt) = &msg {
            if !frame_ok(&sec, txt, txt.as_bytes()) { continue; }
            ...
            if let Ok(ev) = serde_json::from_str::<BinanceStreamEvent>(txt) {
                if let Some(k) = ev.data.kline {
                    ...
                    match k.interval.as_str() {
                        "1h" => { let _ = bus.candles_1h.send(candle); }
                        "4h" => { let _ = bus.candles_4h.send(candle); }
                        _    => {}
                    };
                }
            }
...
struct BinanceStreamEvent { stream: String, data: BinanceKlineWrapper }
...
impl BinanceKline {
    ...
    fn open  (&self) -> f64 { Self::parse_f64(&self.open  ) }
    fn high  (&self) -> f64 { Self::parse_f64(&self.high  ) }
    fn low   (&self) -> f64 { Self::parse_f64(&self.low   ) }
    fn close (&self) -> f64 { Self::parse_f64(&self.close ) }
    fn volume(&self) -> f64 { Self::parse_f64(&self.volume) }
}
